// Inspect the GLB file structure - extract mesh geometry and sample surface points
const fs = require('fs');
const path = require('path');

const glbPath = path.join(__dirname, '../public/glb.glb');
...
function parseGLB(buffer) {
  ...
  // Extract all vertices and indices from all meshes
  const allVertices = [];
  const allFaces = [];
  let vertexOffset = 0;
  ...
  for (const mesh of jsonChunk.meshes || []) {
    ...
    for (const prim of mesh.primitives) {
      ...
      if (prim.indices !== undefined) {
        ...
        const componentType = idxAcc.componentType; // 5123 = UNSIGNED_SHORT, 5125 = UNSIGNED_INT
...
async function main() {
  ...
  console.log('\nCalculating surface areas for weighted sampling...');
  ...
  console.log(`\nSaved ${COUNT} surface-sampled points → logo_points.json (${(fs.statSync(outPath).size/1024).toFixed(0)} KB)`);
}
